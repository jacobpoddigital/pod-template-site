...
| **`SECTION-SPECS.md`** | per-section paired spec — **Payload** (token-resolved values), **Narrative** (intent/hierarchy/mobile), **Decision-tree check** (reuse / `custom_class` / extend-in-template / fork), **Flags** (elements the block can't hold — a to-do, never fold or drop) | **read the matching entry before building each section** — it carries the exact spacing/type/columns/alignment so you don't eyeball the wireframe; the Decision-tree check tells you whether to reuse/extend/fork; a Flag is a to-do you must resolve, not omit |

The brand values in `BUILD-SPEC.md` are **locked** — use the fonts/hex EXACTLY, never substitute.

## The build flow (do this, in order)
1. **Ingest** — read `BUILD-SPEC.md` + `wireframe.html` + `SECTION-SPECS.md`; `cp project-input/tokens.css src/styles/theme.css`; set `site.config.ts` (name, url, nav, footer) + create `.env` from `.env.example` (ask the human for secrets — you cannot write `.env*`).
2. **Per page, in `BUILD-SPEC.md` order** — **read that section's `SECTION-SPECS.md` entry first** (its Payload = the exact values to build to; its Decision-tree check = whether to reuse/extend/fork; its Flags = to-dos you must resolve). Then, for each §6 block not already in the registry, run the **`/new-block`** skill (`.claude/skills/new-block/`): the 4-file slice (`wp/acf-fields/<site>-page-blocks.json` → `schema.ts` → `<block>.tsx` → `registry.tsx`) + the index re-export + the **GraphQL wiring** (a `... on Page_Pagefields_Blocks_<Layout>` fragment in `src/lib/cms/queries/page-by-slug.graphql` + a `LAYOUT` entry in `src/lib/cms/adapters/blocks.ts`) + the **seed** (`wp/provision-content.php`, in wireframe order). Then `pnpm codegen`. Optionally add the block to the dev mock fixture (`src/lib/cms/mock/fixtures.ts`) so it renders offline. Compose blocks from **shadcn primitives** (`src/ui/*`) + the semantic tokens.
...
- **Custom post types** → the template ships `case_study` as the **worked CPT example** (`docs/custom-post-types.md`): registered CPT + its own ACF group (`caseStudyFields`) → typed GraphQL → SSG `/case-studies` routes, rendering offline via the mock. Copy the file set to add another CPT; keep/rename/delete per client.
...
- **Auth / member-only content** → **WPGraphQL-JWT**, server-side `httpOnly`-cookie sessions (no REST; ADR 0013). Gated content is dynamic/uncached. **Scaffolding now SHIPS as an opt-in module** — `/login` · `/forgot-password` · `/reset-password` · a guarded `/account` example, with `getSession`/`requireUser` guards and a CSRF-checked Server Action layer; renders offline via the CMS mock (`docs/auth.md`). CMS auth ops live in `lib/cms` (cms-public); cookie/session glue in `src/app/(auth)/_lib` (the `lib`-only-imports-`lib` boundary). **Go-live gate:** install + verify `wp-graphql-jwt-authentication` vs WPGraphQL 2.x (core reset/viewer already verified) — `docs/auth.md §Go-live`, HQ `workflow/36`.
