...
3. Never write `focus:outline-none` without a visible replacement — always `focus-visible:ring-2 focus-visible:ring-primary`. Use token UTILITIES (`bg-primary`, `rounded-md`, `ring-primary`) — NOT arbitrary `[--var]` reads; `@theme inline` doesn't emit those as `:root` vars, so they resolve to nothing.
