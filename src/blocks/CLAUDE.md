...
1. **ACF layout** — add a layout to the `blocks` Flexible Content field in `wp/acf-export.json` (fields-as-code: a mu-plugin registers it; re-run `./wp/provision.sh` to sync local WP — never edit fields in wp-admin). Layout name is `snake_case`. Set **"Show in GraphQL"** + a pinned GraphQL Type Name per layout.
