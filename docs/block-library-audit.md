...
- [x] **`ui/heading.tsx` + `ui/rich-text.tsx` hand-roll the type scale** ✅ Deleted the dead `ui/heading.tsx`; standardised the whole site onto the type-scale classes (added a `display-xs` card-title token, `text-sm`→`body-sm`, chrome onto the scale). rich-text's injected-HTML variants remain a sanctioned exception. (`text-4xl font-bold`, `[&_h2]:text-3xl [&_h2]:font-bold`, h3 at 600) — the exact ad-hoc pattern the KB/AGENTS.md forbid.
