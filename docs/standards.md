...
- **Icons — always from the library, NEVER a keyboard/unicode glyph.** `lucide-react` for UI glyphs (arrows, checks, chevrons, X…); **`react-icons/fa6` for social brands** (Simple Icons dropped the major social trademarks, so `react-icons/si` lacks them). A literal `→`/`✓`/`★`/`»`/`✕` in JSX or a CSS `content:"→"` is a bug — replace it with the icon component (a CTA's trailing arrow is `<ArrowRight/>`, not `→`; a tick is `<Check/>`, not `✓`). Even when porting a source that uses a glyph, translate it to the library icon. *(House rule, 2026-06-13.)*
- **Button icons** go through the `Button`/`ButtonLink` `icon` + `iconPosition` props (a lucide icon; a trailing icon slides on hover) — don't hand-place an icon inside the label. Default is no icon; pass `icon={ArrowRight}` for a CTA arrow, `iconPosition="leading"` for a leading glyph (download/phone). A brand can default the arrow on per-CTA in its own layer.
...
**Section settings are a DEFINED LIST (`button_group`), not free text.**
- `tone`/`spacing`/`container` are bounded enums in `src/lib/section-settings.ts` (the single source of truth) → the ACF field is a single-choice **`button_group`** whose `choices` mirror the enum, so an editor picks from the list and can't typo a value that 500s. **Do NOT use ACF `select`:** wpgraphql-acf 2.x exposes a `select` as a list (`[String]`), which breaks the scalar `tone spacing container` queries + the `z.enum` schemas. `button_group` (and `radio`) expose as scalar `String`. `anchor` stays `text` (genuinely freeform). Confirm the `String` exposure on the per-project SDL regen — if a field comes back `[String]`, codegen fails **loud**; switch to `radio` or flatten `[0]`. (Free `text` was the old stopgap — it's why a seeded `spacing:"lg"` once 500'd, caught by zod. `docs/conditional-rendering.md §section_settings` has the cookbook.)

**The `reverse` (flip) section setting — standard, in the template.**
- `sectionSettingsFields` includes `reverse: z.boolean().nullish()` + the `flipOrder(reverse)` helper in `src/lib/section-settings.ts` (returns `{a,b}` order classes). A split block reads `flipOrder(reverse)` and applies `order.a`/`order.b` to its two columns (swap at `lg+`, mobile unchanged). Surface it as a `true_false` ACF field (`reverse`, "Flip (content right / media left)"). `media_text` instead uses its richer `media_position` select (it also re-orders on mobile) — use `reverse` for a simple boolean flip on any other split block. The headless equivalent of the old Great White theme's section flip.
...
- **Extend a verified animated engine ADDITIVELY — never refactor its working paths to add a mode.** A new play mode (e.g. hover-to-play for grid mini-demos) goes in as a NEW branch in the effect, placed *after* the existing machinery, leaving the paths that already drive shipped widgets byte-for-byte unchanged; then re-verify the prior widgets still render. A refactor "to make it cleaner" risks regressing every widget the engine already drives. *(Earned: bento mini-demos build, 2026-06-12.)*

**Reusable widgets via a widget slot — NOT a one-off block per placement.**
- A ported/bespoke widget engine is reusable (the block is just a thin `<Section>` wrapper around it). To embed it inside another layout (e.g. a chat demo *beside* hero copy, not stacked below), use the **widget slot** — the headless-native equivalent of a shortcode. Do **NOT** use a literal WP shortcode: parsing `[widget]` out of an HTML string fights `sanitize-html` (§11), carries no typed config, and assumes a prose content model we don't have.
- Primitive (ships generic in the template): `src/widgets/slugs.ts` (the slug list, no client imports, so zod can validate it), `src/widgets/registry.tsx` (`slug → a zero-arg, pre-configured widget`; **empty by default** — a site registers its own bespoke widgets), `src/widgets/widget-slot.tsx` (`<WidgetSlot widget={slug} />`), `src/lib/widget-slot.ts` (`widgetSlotFields` — spread into ANY block's zod to give it an optional embedded widget; a non-empty unknown slug fails LOUD).
- ACF side: the slot is a plain **`text`** field holding the slug — wpgraphql-acf 2.x exposes a `select` as a LIST (`[String]`), so use `text` + zod-validate. *(Earned: Website Navigator home hero, 2026-06-13.)*

**Fidelity pass on a port (matching an existing build).** The method that gets a section *close* to its source lives in `web-ai-automation/workflow/32-ingest-existing-build.md`: screenshot **both** sides at the same viewport · read the source's **actual CSS** (don't eyeball a screenshot — it lies) · map every value to a **design token**, and fix the token where it diverges from source · iterate screenshot → diff → one change → re-screenshot.
